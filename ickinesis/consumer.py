    ...
    def loop(self):
        ...
        try:
            ...
            if exc.response["Error"]["Code"] in RETRY_EXCEPTIONS:
                ...
                loop_status = min((30, (self.retries or 1) * 2))
                log.debug(
                    "Retrying get_records (#%d %ds): %s",
                    self.retries + 1,
                    loop_status,
                    exc,
                )
...
class KinesisConsumer(object):
    ...
    def __init__(
        self, stream_name, boto3_session=None, state=None, reader_sleep_time=None
    ):
    ...
    def setup_shards(self):
        ...
        self.stream_data = self.kinesis_client.describe_stream(
        ...
        for shard_data in self.stream_data["StreamDescription"]["Shards"]:
            ...
            try:
                shard_locked = self.state.lock_shard(
                    self.state_shard_id(shard_data["ShardId"]), self.LOCK_DURATION
                )
                ...
                if not shard_locked:
                    ...
                    if shard_data["ShardId"] in self.shards:
                        log.warn(
                            "We lost our lock on shard %s, stopping shard reader",
                            shard_data["ShardId"],
                        )
            ...
            if shard_data["ShardId"] not in self.shards:
                log.info(
                    "Shard reader for %s does not exist, creating...",
                    shard_data["ShardId"],
                )
                try:
                    iterator_args = self.state.get_iterator_args(
                        self.state_shard_id(shard_data["ShardId"])
                    )
                except AttributeError:
                    # no self.state
                ...
                log.info(
                    "%s iterator arguments: %s", shard_data["ShardId"], iterator_args
                )
    ...
    def __iter__(self):
        try:
            ...
            while self.run:
                ...
                while (
                    self.run and (time.time() - last_setup_check) < lock_duration_check
                ):
                    try:
                        ...
                        for item in resp["Records"]:
                            ...
                            try:
                                self.state.checkpoint(
                                    state_shard_id, item["SequenceNumber"]
                                )
                            except AttributeError:
                                # no self.state
                                pass
                                ...
                                log.exception(
                                    "Unhandled exception check pointing records from %s at %s",
                                    shard_id,
                                    item["SequenceNumber"],
                                )
